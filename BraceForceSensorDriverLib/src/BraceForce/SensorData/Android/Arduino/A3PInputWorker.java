 ...
 * the License.
 */
package BraceForce.SensorData.Android.Arduino;
...
//				}
			...
			switch(currentState){
				...
				case TYP2:
					...
					int tempTypeCode = AndroidUSBCommon.getIntFromTwoBytesLittleEndian(messageType);
				...
				case LEN2:
					...
					payloadLength = AndroidUSBCommon.getIntFromTwoBytesLittleEndian(payloadLengthBytes);
	...
	private byte nextByte() throws IOException {
		...
		return AndroidUSBCommon.getTwoBytesFromInt(nextByteInt)[1];
	...
	private A3PMessage validateAndAssemble(byte[] messageTypeBytes, byte[] messageNumberBytes, byte sensorID,
		...
		if(DEBUG){
			Log.d(LOG_TAG, "Pre-received message# " + AndroidUSBCommon.getIntFromTwoBytesLittleEndian(messageNumberBytes) +
					" \tcrc-calc: 0x" + AndroidUSBCommon.byte2Hex(crcValue) + " \tcrc-rcvd: 0x" + AndroidUSBCommon.byte2Hex(crc));
		}
				
		
		if(crc != crcValue){
			Log.e(LOG_TAG, "CRC Failed! crc calc: " + AndroidUSBCommon.byteToIntUnsigned(crcValue) +
					" crc received: " + AndroidUSBCommon.byteToIntUnsigned(crc));
			return null;
		}
		
		if(DEBUG){
			Log.d(LOG_TAG, "CRC Passed! \t CRC: 0x" + AndroidUSBCommon.byte2Hex(crc));
		}
		...
		// Arduino sends data little-endian
		int messageNumber = AndroidUSBCommon.getIntFromTwoBytesLittleEndian(messageNumberBytes);
