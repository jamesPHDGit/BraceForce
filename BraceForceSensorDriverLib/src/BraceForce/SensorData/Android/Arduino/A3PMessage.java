...
public class A3PMessage {
	...
	public String packetAsString(){
		...
		for (int i = 0; i < payload.length; i++){
			...
			int temp = AndroidUSBCommon.byteToIntUnsigned(payload[i]);
	...
	public byte[] bytesToSend(){
		...
		byte[] messageTypeBytes = AndroidUSBCommon.getTwoBytesFromInt(type.code());
		byte[] messageNumberBytes = AndroidUSBCommon.getTwoBytesFromInt(messageNumber);
		byte[] payloadLengthBytes = AndroidUSBCommon.getTwoBytesFromInt(payload.length);
