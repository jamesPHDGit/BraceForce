...
public class DatabaseManager {
	 ...
	 */
	public synchronized List<SensorMetaData> sensorList( CommunicationChannelType type ) {
		SQLiteDatabase db = mOpenHelper.getReadableDatabase();
		...
		// parse results
		ArrayList<SensorMetaData> results = new ArrayList<SensorMetaData>();
		while (cursor.moveToNext()) {
			SensorMetaData data = new SensorMetaData();
	...
	public synchronized void deleteAllSensors() {
		for(CommunicationChannelType type : CommunicationChannelType.values()) {
			for(SensorMetaData sensor : sensorList(type)) {
