 ...
 * 
 */
public class AndroidSubChannel implements USBCommSubChannel, Runnable {
	...
	public AndroidSubChannel (Context svcContext,AndroidSensorDataManager sensorManager) {
...
//    }
	...
	public void run() {
		String LOG_TAG = AndroidSubChannel.TAG + "worker";
				...
				if(nextPayload == null){
					...
					else 
					{
						...
						if(nextPayload.isReadingSeries()) {
							sdp = new AndroidSensorDataPacket(nextPayload.getRawBytes(), nextPayload.getSeriesTimestamp(), nextPayload.getNumOfReadingsInSeries());
						} else {
							sdp = new AndroidSensorDataPacket(nextPayload.getRawBytes(), nextPayload.getAndroidTimeStamp());
						}
